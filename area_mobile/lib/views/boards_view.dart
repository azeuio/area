...
class _BoardsViewState extends State<BoardsView> {
  ...
  late Future<List<dynamic>?> userBoardsFuture = getCurrentUser().then((User? user) {
    setState(() {
      this.user = user;
    });
    return getUserBoards(user, 'http://10.0.2.2:8080');
  });
                 ...
                 if (userBoards != null)
                   for (var board in userBoards!)
                    Container(
                      ...
                      child: BoardCard(
                        backgroundColor: Color(int.parse(board['color'].substring(1, 7), radix: 16) + 0xFF000000),
                        title: board['name'],
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => BoardView(boardId: board['id']),
                            ),
                          );
                        },
                      ),

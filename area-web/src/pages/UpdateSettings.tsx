...
import { UpdateSettingsStatus, isSignedWithPassword, tryUpdateUsername, tryUpdateEmail, tryUpdatePassword, deleteAccount } from './UpdateSettingsLogic';
...
function UpdateSettings() {
    ...
    const [userWantToDeleteAccount, setUserWantToDeleteAccount] = useState<boolean>(false);
    const [updateEmailStatus, setUpdateEmailStatus] = useState<UpdateSettingsStatus>(UpdateSettingsStatus.NotUpdated);
    const [updatePasswordStatus, setUpdatePasswordStatus] = useState<UpdateSettingsStatus>(UpdateSettingsStatus.NotUpdated);
    ...
    const handleSave = async () => {
        await tryUpdateUsername(backendUrl, currentUserToken, actualUsername, newUsername);
        const emailStatus = await tryUpdateEmail(backendUrl, currentUserToken, actualEmail, newEmail);
        const passwordStatus = await tryUpdatePassword(backendUrl, actualPassword, newPassword);

        setUpdateEmailStatus(emailStatus);
        setUpdatePasswordStatus(passwordStatus);
        if (emailStatus != UpdateSettingsStatus.NotUpdated || passwordStatus === UpdateSettingsStatus.Error) {
            return;
        }
        navigate('/profile');
    }
    ...
    return (
        <div className="h-screen flex flex-col items-center flex-grow justify-center space-y-10">
            ...
            <Modal
                ...
                onClose={() => {
                    if (updatePasswordStatus === UpdateSettingsStatus.Error || updateEmailStatus === UpdateSettingsStatus.Error) {
                        setUpdateEmailStatus(UpdateSettingsStatus.NotUpdated);
                    } else {
                        navigate('/profile');
                    }
                }}
                children={updateEmailStatus === UpdateSettingsStatus.Updated ?
                    <p>Your email has been updated. Please check your inbox to confirm your new email.</p> :
                    <p>Error when updating your email. Please try again.</p>
                }
            />
            <Modal
                isOpen={updatePasswordStatus === UpdateSettingsStatus.Error && updateEmailStatus === UpdateSettingsStatus.NotUpdated}
